...
# (получение tgt) 
		nxc smb -u nathan.aadam -p '3edc#EDC3' -d mirage.htb -k --generate-tgt nathan.aadam dc01.mirage.htb
				
# (Нахождение делегирования)  
		nxc ldap  10.10.11.78 -u mark.bbond -p '1day@atime' -k --trusted-for-delegation --find-delegation   
		
# (Нахождение доверительных доменов)	
		nxc ldap -u john.w -p 'RFulUtONCOL!' -d darkzero.htb dc01.darkzero.htb --dc-list
		
# (поиск центра сертификации)	
		crackmapexec ldap 'dc.sequel.htb' -d 'sequel.htb' -u 'Ryan.Cooper' -p 'NuclearMosquito3' -M adcs  	
  		
# (Уязвимые для атак перенаправления серверы )	
		nxc smb 10.10.1.50 -u kemг -p password -M coersce_plus		
